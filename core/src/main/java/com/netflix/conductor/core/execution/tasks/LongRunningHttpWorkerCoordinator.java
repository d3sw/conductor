 ...
 */
@Singleton
public class LongRunningHttpWorkerCoordinator {

	private static Logger logger = LoggerFactory.getLogger(LongRunningHttpWorkerCoordinator.class);
	...
	private static final String className = LongRunningHttpWorkerCoordinator.class.getName();
	...
	@Inject
	public LongRunningHttpWorkerCoordinator(QueueDAO taskQueues, WorkflowExecutor executor, Configuration config) {
		...
		if(threadCount > 0) {
			...
			logger.debug("Long Running Http Worker Initialized with {} threads and a callback time of {} second and queue size {} with pollCount {}", threadCount, unackTimeout, workerQueueSize, pollCount);
		} else {
			logger.warn("Long Running Http Worker DISABLED");
		}
		try {
	...
	private void pollAndExecute(WorkflowSystemTask systemTask) {
		try {
			...
			List<String> polled = taskQueues.getQueueRecords(name, effectivePollCount, pollTimeout);
			...
			for(String task : polled) {
				try {
					es.submit(()-> {
						...
						try {
							executor.executeSystemTaskUpdate(systemTask, task);
						} finally {
							NDC.remove();
						}
					});
